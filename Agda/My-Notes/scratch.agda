...
&&-is-distributive : (a b c : Bool) → (a && b || a && c) ≡ a && (b || c)
&&-is-distributive true b c = refl (b || c)
&&-is-distributive false b c = refl false

∔-is-distributive : {A B C : Type} → A × B ∔ A × C → A × (B ∔ C)
∔-is-distributive {A} {B} {C} = {!A!}

data Suit : Type where
  hearts clubs diamonds spades : Suit

thing : {P : Suit → Type} (s : Suit) → P s
thing {P} hearts = {!!}
thing {P} clubs = {!!}
thing {P} diamonds = {!!}
thing {P} spades = {!!}
